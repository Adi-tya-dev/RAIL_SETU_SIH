...
async function findById(id) {
  ...
  return prisma.blockPlan.findUnique({
...
async function simulate(id, body = {}) {
  ...
  const tasks = plan.plan_maintenance_tasks.map((link) => link.maintenance_task);
  if (!tasks.some((task) => String(task.maintenance_task_id) === String(taskId))) {
    throw new ApiError(400, "Task is not part of this plan");
  }

  const movements = await prisma.trainBlockMovement.findMany({
